...
export default function MinimalChatInterface({ businessId }: MinimalChatInterfaceProps) {
    ...
    const loadConversations = async () => {
      ...
      try {
        ...
        if (conversationsData && conversationsData.length > 0) {
          ...
          // Para cada conversación, obtener el nombre más reciente del backend
          const updatedConvsPromises = sortedConversations.map(async (conv: any) => {
            ...
            try {
              ...
              console.log(`[DEBUG] Conversación ${conv.id} - name final:`, name);
  ...
  useEffect(() => {
    ...
    const loadMessages = async () => {
      try {
        ...
        console.log(`✅ Obtenidos ${fetchedMessages.length} mensajes para conversación ${chatId}`);
  ...
  const confirmDeleteConversation = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "La conversación fue eliminada correctamente.",
        duration: 2000
      });
    } catch (error: any) {
  ...
  const handleUpdateName = useCallback((id: string, name: string) => {
    ...
    // Si el chat seleccionado es el que se actualiza, actualizar también su nombre
  ...
  // Antes del return, define la función para eliminar una conversación del estado global
  ...
  // Función para limpiar localStorage y estado de conversaciones huérfanas
  const cleanOrphanConversations = useCallback((validIds: string[]) => {
    console.log('[DEBUG] IDs válidos desde backend:', validIds);
    ...
    setConversations(prev => {
      ...
      console.log('[DEBUG] Conversations después de limpiar:', cleaned.map(c => c.id));
      return cleaned;
    });
  }, []);
              ...
              onClick={async () => {
                ...
                try {
                  if (loadConversationsRef.current) {
                    ...
                    toast({
                      title: "Error",
                      description: "No se pudo recargar las conversaciones porque no hay un businessId válido.",
                      variant: "destructive"
                    });
                  }
                } catch (error: any) {
